...
public:
    ...
    void print(auto c, auto d)
    {
        std::cout << c << std::endl;
        
    }
    void println(auto c, auto d)
    {
        std::cout << c << std::endl;
...
int base64_decode(char * output, char * input, int inputLen)
{
	...
	int payLength = b64_to_bin(input, strlen(input), (uint8_t *)output, 1024);
...
int sendPacket(uint8_t *buff_down, uint8_t length) 
{
    ...
    */
	...
	if (payLength != psize) {
		...
		for (int i=0; i<payLength; i++) {Serial.print(payLoad[i],HEX); Serial.print(':'); }
		Serial.println();
	}
